...
def main():
    ...
    # Open Serial port
    try:
        arduino = Serial(PORT, 2000000) #2000000)
    ...
    arduino.flushInput()
    arduino.write(b'HX') #Rajeshwar_corrected
    if arduino.read() == b'H':
        ...
        if not mcu_found:
            arduino.flushInput()
            arduino.write(b'DX')  # asking Arduino for the DeviceID #Rajeshwar_corrected
        ...
        if mcu_found:
            ...
            arduino.flushInput()
            arduino.write(b'EX') #rajeshwar_corrected
            if arduino.read() == b"K":
                print("\tSuccess")
                ...
                if not ERASE_MODE:
                    ...
                    while address < 0x8000:
                        if hexFile.haveData(address):
                            ...
                            #bytes.fromhex(hex(address)[2:].zfill(4))
                            for j in range(0x20):
                                buf += str(hex(hexFile.getData(address + j))[2:].zfill(2)) 
                                #bytes.fromhex(hex(hexFile.getData(address + j))[2:].zfill(2))
                            buf += "X" #Rajeshwar_curr #Err
                            if verbose:
                                print(buf)
                            arduino.flushInput()
                            arduino.write(buf.upper().encode()) #buf.upper())
                            arduino.read()
                        address += 0x20
                    print("\tSuccess")
                    ...
                    if hexFile.haveID():
                        ...
                        if verbose:
                            print(buf)
                        arduino.flushInput()
                        arduino.write(buf.upper().encode())
                        arduino.read()
                        print("\tSuccess")
                    ...
                    while address < 0x8000:
                        if hexFile.haveData(address):
                            ...
                            buf = ""
                            r = arduino.read()
                            while r != b"X": #Rajeshwar_corrected
                                buf += r.decode() #buf +=r
                                r = arduino.read()
                            buf += r.decode() # buf += r
